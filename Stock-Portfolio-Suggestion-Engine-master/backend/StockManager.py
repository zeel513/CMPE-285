...
def get_latest_price(strategies):
    ...
    response = requests.get(base_url + url_filter+ '?apikey=8eb2f845c8538dcfa32da8f7faea140d')
...
def get_weekly_trend(strategies, allocation):
    ...
    for strategy in strategies:
        for ticker in stocks.get(strategy):
            url = base_url + ticker + date_filter + '&apikey=8eb2f845c8538dcfa32da8f7faea140d'
            print(url)
